...
Ttest = tall(testImds);
trainfeatures = cellfun(@(x)feature_extraction_dct(x,NB),Ttrain,'Uni',0);
testfeatures = cellfun(@(x)feature_extraction_dct(x,NB),Ttest,'Uni',0);
...
title(['DCT, NN, accuracy = ',num2str(Accuracy),'%']);
...
figure;
cchart = confusionchart(testImds.Labels,predlabels);
title(['DCT, SVM, accuracy = ',num2str(Accuracy),'%']);
[metrics, ~] = multiclass_metrics_special(cchart.NormalizedValues);
disp(table(Time,...
    'RowNames',{'Time'},'VariableNames',{'Seconds'}));
disp(table([metrics.Accuracy;metrics.Recall;metrics.Precision;metrics.F1_score]*100,...
